...
-opaque set(CYR) :: {set, gleam@dict:dict(CYR, list(nil))}.
...
-spec contains(set(CZC), CZC) -> boolean().
contains(Set, Member) ->
...
-spec delete(set(CZE), CZE) -> set(CZE).
delete(Set, Member) ->
...
-spec fold(set(CZN), CZP, fun((CZP, CZN) -> CZP)) -> CZP.
...
-spec filter(set(CZQ), fun((CZQ) -> boolean())) -> set(CZQ).
...
-spec drop(set(CZX), list(CZX)) -> set(CZX).
drop(Set, Disallowed) ->
...
-spec take(set(DAB), list(DAB)) -> set(DAB).
take(Set, Desired) ->
    {set, gleam@dict:take(erlang:element(2, Set), Desired)}.

-file("src/gleam/set.gleam", 287).
-spec order(set(DAJ), set(DAJ)) -> {set(DAJ), set(DAJ)}.
order(First, Second) ->
...
-spec intersection(set(DAO), set(DAO)) -> set(DAO).
...
-spec difference(set(DAS), set(DAS)) -> set(DAS).
...
-spec is_subset(set(DAW), set(DAW)) -> boolean().
...
-spec is_disjoint(set(DAZ), set(DAZ)) -> boolean().
...
-spec each(set(DBG), fun((DBG) -> any())) -> nil.
each(Set, Fun) ->
    fold(
        Set,
        nil,
...
-spec insert(set(CYZ), CYZ) -> set(CYZ).
insert(Set, Member) ->
...
-spec from_list(list(CZK)) -> set(CZK).
from_list(Members) ->
...
-spec map(set(CZT), fun((CZT) -> CZV)) -> set(CZV).
map(Set, Fun) ->
...
-spec union(set(DAF), set(DAF)) -> set(DAF).
union(First, Second) ->
    {Larger, Smaller} = order(First, Second),
...
-spec symmetric_difference(set(DBC), set(DBC)) -> set(DBC).
