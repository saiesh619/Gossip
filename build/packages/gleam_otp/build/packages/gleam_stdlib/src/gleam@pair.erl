...
-spec map_first({COR, COS}, fun((COR) -> COT)) -> {COT, COS}.
map_first(Pair, Fun) ->
    {A, B} = Pair,
    {Fun(A), B}.
...
-spec map_second({COU, COV}, fun((COV) -> COW)) -> {COU, COW}.
map_second(Pair, Fun) ->
    {A, B} = Pair,
    {A, Fun(B)}.
