...
-opaque decoder(BXR) :: {decoder,
        fun((gleam@dynamic:dynamic_()) -> {BXR, list(decode_error())})}.
...
-spec run(gleam@dynamic:dynamic_(), decoder(BXZ)) -> {ok, BXZ} |
...
-spec map(decoder(CBX), fun((CBX) -> CBZ)) -> decoder(CBZ).
...
-spec then(decoder(CCJ), fun((CCJ) -> decoder(CCL))) -> decoder(CCL).
then(Decoder, Next) ->
    {decoder,
        fun(Dynamic_data) ->
...
-spec one_of(decoder(CCO), list(decoder(CCO))) -> decoder(CCO).
...
-spec recursive(fun(() -> decoder(CDE))) -> decoder(CDE).
recursive(Inner) ->
    {decoder,
        fun(Data) ->
            Decoder = Inner(),
...
-spec optional(decoder(CBT)) -> decoder(gleam@option:option(CBT)).
optional(Inner) ->
...
-spec run_dynamic_function(
    gleam@dynamic:dynamic_(),
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, BZU} | {error, BZU})
) -> {BZU, list(decode_error())}.
...
-spec collapse_errors(decoder(CCG), binary()) -> decoder(CCG).
...
-spec failure(CCY, binary()) -> decoder(CCY).
...
-spec new_primitive_decoder(
    binary(),
    fun((gleam@dynamic:dynamic_()) -> {ok, CDA} | {error, CDA})
) -> decoder(CDA).
...
-spec fold_dict(
    {gleam@dict:dict(CBF, CBG), list(decode_error())},
    gleam@dynamic:dynamic_(),
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_()) -> {CBF, list(decode_error())}),
...
-spec dict(decoder(CAY), decoder(CBA)) -> decoder(gleam@dict:dict(CAY, CBA)).
dict(Key, Value) ->
...
-spec list(decoder(CAM)) -> decoder(list(CAM)).
list(Inner) ->
    {decoder,
        fun(Data) ->
            gleam_stdlib:list(
...
-spec push_path({BYV, list(decode_error())}, list(any())) -> {BYV,
    list(decode_error())}.
...
-spec index(
    ...
    gleam@dynamic:dynamic_(),
    fun((gleam@dynamic:dynamic_(), list(BYJ)) -> {BYM, list(decode_error())})
) -> {BYM, list(decode_error())}.
...
-spec subfield(list(any()), decoder(BXU), fun((BXU) -> decoder(BXW))) -> decoder(BXW).
...
-spec at(list(any()), decoder(BYG)) -> decoder(BYG).
at(Path, Inner) ->
    {decoder,
        fun(Data) ->
            index(
...
-spec field(any(), decoder(BZE), fun((BZE) -> decoder(BZG))) -> decoder(BZG).
...
-spec optional_field(any(), BZK, decoder(BZK), fun((BZK) -> decoder(BZM))) -> decoder(BZM).
...
-spec optionally_at(list(any()), BZR, decoder(BZR)) -> decoder(BZR).
