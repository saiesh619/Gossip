...
-spec window_loop(list(list(AON)), list(AON), integer()) -> list(list(AON)).
...
-spec window_by_2(list(AOT)) -> list({AOT, AOT}).
window_by_2(List) ->
...
-spec drop_while(list(AOW), fun((AOW) -> boolean())) -> list(AOW).
...
-spec take_while_loop(list(APC), fun((APC) -> boolean()), list(APC)) -> list(APC).
...
-spec take_while(list(AOZ), fun((AOZ) -> boolean())) -> list(AOZ).
take_while(List, Predicate) ->
    take_while_loop(List, Predicate, []).

-file("src/gleam/list.gleam", 1993).
-spec chunk_loop(list(APL), fun((APL) -> APN), APN, list(APL), list(list(APL))) -> list(list(APL)).
...
-spec chunk(list(APG), fun((APG) -> any())) -> list(list(APG)).
chunk(List, F) ->
    case List of
        [] ->
            [];
...
-spec sized_chunk(list(APT), integer()) -> list(list(APT)).
...
-spec reduce(list(AQE), fun((AQE, AQE) -> AQE)) -> {ok, AQE} | {error, nil}.
reduce(List, Fun) ->
    case List of
        [] ->
            {error, nil};
...
-spec scan_loop(list(AQM), AQO, list(AQO), fun((AQO, AQM) -> AQO)) -> list(AQO).
...
-spec last(list(AQR)) -> {ok, AQR} | {error, nil}.
last(List) ->
    case List of
        [] ->
            {error, nil};
...
-spec combinations(list(AQV), integer()) -> list(list(AQV)).
...
-spec combination_pairs_loop(list(ARC), list({ARC, ARC})) -> list({ARC, ARC}).
...
-spec combination_pairs(list(AQZ)) -> list({AQZ, AQZ}).
combination_pairs(Items) ->
    combination_pairs_loop(Items, []).

-file("src/gleam/list.gleam", 2241).
-spec take_firsts(list(list(ARW)), list(ARW), list(list(ARW))) -> {list(ARW),
    list(list(ARW))}.
...
-spec transpose_loop(list(list(ARP)), list(list(ARP))) -> list(list(ARP)).
...
-spec transpose(list(list(ARK))) -> list(list(ARK)).
...
-spec interleave(list(list(ARG))) -> list(ARG).
interleave(List) ->
    _pipe = transpose(List),
    lists:append(_pipe).

-file("src/gleam/list.gleam", 2274).
-spec shuffle_pair_unwrap_loop(list({float(), ASI}), list(ASI)) -> list(ASI).
...
-spec max_loop(list(ASW), fun((ASW, ASW) -> gleam@order:order()), ASW) -> ASW.
...
-spec max(list(ASP), fun((ASP, ASP) -> gleam@order:order())) -> {ok, ASP} |
    {error, nil}.
max(List, Compare) ->
    case List of
        [] ->
...
) -> {gleam@dict:dict(integer(), ATL), list(ATL)}.
...
-spec build_reservoir(list(ATG), integer()) -> {gleam@dict:dict(integer(), ATG),
    list(ATG)}.
...
-spec sample(list(ASX), integer()) -> list(ASX).
sample(List, N) ->
...
-spec permutation_zip(list(ANT), list(ANT), list(list(ANT))) -> list(list(ANT)).
